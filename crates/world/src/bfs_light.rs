...
impl<'a, C: ChunkAccess> BfsLight<'a, C> {
    ...
    pub fn add(&mut self, node: LightNode) {
        ...
        self.addition_queue.push_back(node);
    }

    pub fn add_custom(&mut self, node: LightNode, light_level: u8) {
        self.starting_chunk = Some(node.1);
        self.chunk_manager.set_local_light(node.1, node.0, self.is_sky_light, light_level);
        self.addition_queue.push_back(node);
    }
    ...
    pub fn remove(&mut self, node: LightNode) {
        ...
        let light_level = self.chunk_manager.get_local_light(node.1, node.0, self.is_sky_light);

        self.removing_queue.push((node, light_level));
        self.chunk_manager
            .set_local_light(node.1, node.0, self.is_sky_light, if self.is_sky_light && node.0.y == 255 { 15 } else { 0 });
    }
    ...
    pub fn calculate_with_info<T: BlockSource>(&mut self, block_source: &T) -> Vec<IPoint2D> {
                ...
                if let Some(chunk) = self.chunk_manager.get_chunk_by_block_mut(neighbour_pos) {
                    ...
                    if local_position.y < CHUNK_HEIGHT {
                            ...
                            || (neighbour_light_level != 0 && neighbour_light_level < node_light_level)
                        {
                            ...
                            self.addition_queue.push_back(LightNode(local_position, chunk.origin));
                        }
                    }
                }
            }
        ...
        while let Some(node) = self.addition_queue.pop_front() {
            if self.starting_chunk != Some(node.1) && !other_chunks_affected.contains(&node.1) {
                other_chunks_affected.push(node.1);
            }
            ...
            let light_level = self.chunk_manager.get_local_light(node.1, local_position, self.is_sky_light);

            for face in [Face::Left, Face::Right, Face::Back, Face::Front, Face::Bottom, Face::Top] {
                let (chunk, position) = Chunk::to_origin_and_local(world_position + face.as_normal().to_vector());
